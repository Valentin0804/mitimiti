...
function deletePerson(name) {
  ...
  const hasConsumedExpenses = expenses.some((expense) =>
    expense.consumedBy.includes(name)
  );
...
function addExpense() {
  ...
  const consumedByCheckboxes = document.querySelectorAll(
    '#consumedByCheckboxes input[type="checkbox"]'
  );
...
function renderSummary() {
  ...
  people.forEach((person) => {
    ...
    if (amountToPay < 0) {
      amountToPayFormatted = `<span style="color: red;">-$${Math.abs(
        amountToPay.toFixed(2)
      )}</span>`;
    } else {
      amountToPayFormatted = `<span style="color: green;">$${amountToPay.toFixed(
        2
      )}</span>`;
    }
...
function renderExpensesTable() {
  ...
  expenses.forEach((expense, index) => {
    ...
    row.innerHTML = `
            ...
            <td><button class="delete-btn-2"  onclick="deleteExpense(${index})"><i class="fa fa-trash"></i></button></td>
        `;
...
function renderExpenses() {
  ...
  expenses.forEach((expense, index) => {
    ...
    li.textContent = `${expense.description} - $${expense.amount.toFixed(
      ...
      expense.person
    } - Consumido por: ${expense.consumedBy.join(", ")}`;
...
function deleteExpense(index) {
  ...
  const amountPerConsumer =
    deletedExpense.amount / deletedExpense.consumedBy.length;

  consumers.forEach((consumer) => {
    const consumerIndex = people.findIndex(
      (person) => person.name === consumer
    );
    if (consumerIndex !== -1) {
